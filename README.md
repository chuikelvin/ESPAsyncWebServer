...
request->method();        // enum:    HTTP_ASYNC_GET, HTTP_ASYNC_POST, HTTP_ASYNC_DELETE, HTTP_ASYNC_PUT, HTTP_ASYNC_PATCH, HTTP_ASYNC_HEAD, HTTP_ASYNC_OPTIONS
...
server.on("/scan", HTTP_ASYNC_GET, [](AsyncWebServerRequest *request){
  String json = "[";
...
void setup(){
  ...
  server.on("/heap", HTTP_ASYNC_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", String(ESP.getFreeHeap()));
  });

  // upload a file to /upload
  server.on("/upload", HTTP_ASYNC_POST, [](AsyncWebServerRequest *request){
    request->send(200);
  }, onUpload);

  // send a file when /index is requested
  server.on("/index", HTTP_ASYNC_ANY, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/index.htm");
  });

  // HTTP basic authentication
  server.on("/login", HTTP_ASYNC_GET, [](AsyncWebServerRequest *request){
  ...
  server.on("/update", HTTP_ASYNC_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>");
  });
  server.on("/update", HTTP_ASYNC_POST, [](AsyncWebServerRequest *request){
...
public :
  ...
  void begin(){
    ...
    classWebServer.on("/example", HTTP_ASYNC_ANY, handleRequest);

    // attach class request handler
    classWebServer.on("/example", HTTP_ASYNC_ANY, std::bind(&WebClass::classRequest, this, std::placeholders::_1));
  }
};
...
void setup() {
  // attach global request handler
  globalWebServer.on("/example", HTTP_ASYNC_ANY, handleRequest);

  // attach class request handler
  globalWebServer.on("/example", HTTP_ASYNC_ANY, std::bind(&WebClass::classRequest, webClassInstance, std::placeholders::_1));
}
...
webServer.onNotFound([](AsyncWebServerRequest *request) {
  if (request->method() == HTTP_ASYNC_OPTIONS) {
    request->send(200);
  } else {
    request->send(404);
  }
...
```cpp
  server.on("^\\/sensor\\/([0-9]+)$", HTTP_ASYNC_GET, [] (AsyncWebServerRequest *request) {
